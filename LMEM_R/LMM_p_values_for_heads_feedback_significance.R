...
# save tables with p-values, which are used to build topomaps: color topomaps on which sensors with reliable significance of the difference between "feedback positive" vs "feedback negative" for each choice (trial) type
...
#### создает таблицу со значениями pvalue для контрастов "feedback positive" vs "feedback negative" для различных choice (trial) type
###### feedback_cur #######
p_vals <- data.table()
...
write.csv(p_vals, paste0(out_path, "p_vals_Tukey_by_feedback_cur_MEG.csv"))
