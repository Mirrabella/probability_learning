...
def make_beta_signal(subj, r, cond, fb, data_path, L_freq, H_freq, f_step, period_start, period_end, baseline, n_cycles, events, events_response, time_bandwidth = 4):
...
def ttest_vs_zero(data_path, subjects, parameter1, freq_range, planar, n): # n - количество временных отчетов
	...
	for ind, subj in enumerate(subjects):
		temp1 = mne.Evoked(op.join(data_path, '{0}_{1}_evoked_{2}_trf_early_log_resp_{3}.fif'.format(subj, parameter1, freq_range, planar)))
