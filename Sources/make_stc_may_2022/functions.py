...
###########################################################################################################################
############################################# stc for baseline ############################################################    
def make_stc_epochs_for_baseline(subj, r, data_path, L_freq, H_freq, f_step, baseline, bem, src, events, trans):
    ...
    stc_epo_bl_list = []
    for ix in range(len(epochs_bl)):
        stc_baseline = mne.minimum_norm.source_band_induced_power(epochs_bl[ix].pick('meg'), inv, bands, method='sLORETA', use_fft=False, df = f_step, n_cycles = 8)["beta"].crop(tmin=baseline[0], tmax=baseline[1], include_tmax=True)
        
        #data = 10*np.log10(stc_baseline.data) # make log transformation
        #stc.data = 10*np.log10(stc.data/b_line[:, np.newaxis])
        #stc.data = data
        stc_epo_bl_list.append(stc_baseline)
    return (stc_epo_bl_list)
