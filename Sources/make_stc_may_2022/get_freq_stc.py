...
import numpy as np
import pandas as pd
from func import make_stc_epochs
...
description = 'STC for beta power 16 - 30 Hz, epochs, без поправки на бейзлайн и без логарифмирования и умножения на 10'
...
with open("/net/server/data/Archive/prob_learn/data_processing/beta_16_30_sources/stc_by_epo_no_baseline_sLoreta/config.txt".format(freq_range), "w") as file:
    for  line in lines:
        file.write(line + '\n')
            ...
            for fb in feedback:
                ...
                if events.size == 0:
                    ...
                    try:
                        ...
                        stc_epo_list = make_stc_epochs(subj, r, cond, fb, data_path, L_freq, H_freq, f_step, period_start, period_end, baseline, bem, src, events, events_response, trans)
                        print('Количество эпох %s' % len(stc_epo_list))
                                
                        os.makedirs('/net/server/data/Archive/prob_learn/data_processing/beta_16_30_sources/stc_by_epo_no_baseline_sLoreta/{1}_run{2}_{3}_fb_cur_{4}'.format(freq_range, subj, r, cond, fb))
                                
                        for s in range(len(stc_epo_list)):
                            stc_epo_list[s].save('/net/server/data/Archive/prob_learn/data_processing/beta_16_30_sources/stc_by_epo_no_baseline_sLoreta/{1}_run{2}_{3}_fb_cur_{4}/{5}'.format(freq_range, subj, r, cond, fb, s))
