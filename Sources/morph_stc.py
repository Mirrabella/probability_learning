...
os.makedirs('/net/server/data/Archive/prob_learn/vtretyakova/sources/{0}/{0}_stc_epo_var2_no_morph_fsaverage'.format(freq_range), exist_ok = True)
        ...
        for cond in trial_type:
            for fb in feedback:
                try:
                    ...
                    stc= mne.read_source_estimate("/net/server/data/Archive/prob_learn/vtretyakova/sources/{0}/{0}_stc_epo_average_epo_var2_no_morph/{1}_run{2}_{3}_fb_cur_{4}".format(freq_range, subj, r, cond, fb))
                    morph = mne.compute_source_morph(stc, subject_from=subj, subject_to='fsaverage')
                    stc_fsaverage = morph.apply(stc)
                    stc_fsaverage.save('/net/server/data/Archive/prob_learn/vtretyakova/sources/{0}/{0}_stc_epo_var2_no_morph_fsaverage/{1}_run{2}_{3}_fb_cur_{4}_fsaverage'.format(freq_range, subj, r, cond, fb))
