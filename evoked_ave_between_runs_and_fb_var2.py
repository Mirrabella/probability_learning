...
for t in trial_type:
    ...
    for subj in subjects:
        ...
        for r in rounds:
            ...
            try:   
                epochs_negative = mne.read_epochs('/net/server/data/Archive/prob_learn/vtretyakova/Nikita_mio_cleaned/beta_16_30_epo/{0}_run{1}_{1}_fb_cur_negative_beta_16_30-epo.fif'.format(subj, r, t), preload = True)
            ...
            # если оба фид бека есть у испытуемого, то объединяем их
            if p is True and n is True:
                ...
                # получаем усредненные данные для одного блока
            ...
            # если только положительные, то в эвокд идет только положительный фид бек
            elif p is True and n is False:
                epochs = epochs_positive
                evoked = epochs.get_data().mean(axis=0)
                evoked_internal_subjects.append(evoked)
                
                # если только отрицательный, то в эвокд идет только отрицательный фид бек
            elif p is False and n is True:
                epochs = epochs_negative
                evoked = epochs.get_data().mean(axis=0)
                evoked_internal_subjects.append(evoked)
                    
                # если в данном блоке вообще нет трайлов, то пропускаем его    
            else:
                pass
                #except (OSError):
                #    print('This file not exist')
                        
        print('evoked %s'%len(evoked_internal_subjects))
