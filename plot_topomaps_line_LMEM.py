...
for p in planars:
    _, _, risk_mean, norisk_mean = ttest_pair(data_path, subjects, parameter1 = 'risk', parameter2 = 'norisk', planar = p,  n = n)


    ########################### p value #############################
    # загружаем таблицу с pvalue, полученными с помощью LMEM в R
    df = pd.read_csv('/home/vtretyakova/Рабочий стол/probability_learning/p_vals_Tukey_by_trial_type_MEG_{0}.csv'.format(p))
    ...
    title = ('norisk vs risk, %s, LMEM, noFDR'%p)
    ...
    title = ('norisk vs risk, %s, LMEM,  space FDR'%p)
    ...
    title = ('norisk vs risk, %s, LMEM, full FDR'%p)
    ...
    fig.savefig('/net/server/data/Archive/prob_learn/vtretyakova/Nikita_mio_cleaned/topomaps_lines/norisk_vs_risk/LMEM_norisk_vs_risk_stat_no_fdr_{0}_separ_fb.jpeg'.format(p), dpi = 300)

    fig2.savefig('/net/server/data/Archive/prob_learn/vtretyakova/Nikita_mio_cleaned/topomaps_lines/norisk_vs_risk/LMEM_norisk_vs_risk_stat_space_fdr_{0}_separ_fb.jpeg'.format(p), dpi = 300)


    fig3.savefig('/net/server/data/Archive/prob_learn/vtretyakova/Nikita_mio_cleaned/topomaps_lines/norisk_vs_risk/LMEM_norisk_vs_risk_stat_full_fdr_{0}_separ_fb.jpeg'.format(p), dpi = 300)
...
for p in planars:
    _, _, prerisk_mean, norisk_mean = ttest_pair(data_path, subjects, parameter1 = 'prerisk', parameter2 = 'norisk', planar = p, n = n)
    ...
    fig.savefig('/net/server/data/Archive/prob_learn/vtretyakova/Nikita_mio_cleaned/topomaps_lines/norisk_vs_prerisk/LMEM_norisk_vs_prerisk_stat_no_fdr_{0}_separ_fb.jpeg'.format(p), dpi = 300)

    fig2.savefig('/net/server/data/Archive/prob_learn/vtretyakova/Nikita_mio_cleaned/topomaps_lines/norisk_vs_prerisk/LMEM_norisk_vs_prerisk_stat_space_fdr_{0}_separ_fb.jpeg'.format(p), dpi = 300)


    fig3.savefig('/net/server/data/Archive/prob_learn/vtretyakova/Nikita_mio_cleaned/topomaps_lines/norisk_vs_prerisk/LMEM_norisk_vs_prerisk_stat_full_fdr_{0}_separ_fb.jpeg'.format(p), dpi = 300)
